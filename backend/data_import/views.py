...
from .permissions import CanDeleteDatasets, IsDatasetOwner
from .serializers import DataImportProcessSerializer, DataImportRequestSerializer
...
class ImportDataView(APIView):
    ...
    def post(self, request):
        ...
        logger.info("FILES: {list(request.FILES.keys())}")
        logger.info("DATA keys: {list(request.data.keys())}")
        ...
        logger.info("   - Tipo: {import_type}")
        logger.info("   - Tabela: {table_name}")
        if import_type == "file" and uploaded_file:
            logger.info("   - Arquivo: {uploaded_file.name}")
        ...
        try:
            ...
            if "Erro ao ler arquivo" in error_message:
                ...
                user_message = (
                    "Formato de arquivo não suportado. Use arquivos .xlsx, .xls ou .csv"
                )
                ...
                user_message = (
                    "Não foi possível conectar ao servidor de dados. Verifique a URL."
                )
            else:
...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter(
            name="page_size", type=int, description="Itens por página (max 100)"
        ),
    ],
...
class ProcessDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"error": "Processo não encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
...
class DeleteProcessView(APIView):
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            logger.info(
                f"[OK] Processo {table_name} e {record_count} registros deletados com sucesso"
            )
            ...
            return Response(
                {"error": "Erro ao deletar processo: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class AppendDataView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info("[APPEND] Adicionando dados a tabela: {process.table_name}")
            logger.info("=" * 60)
            ...
            if import_type == "endpoint":
                ...
                data, column_structure = DataImportService.process_file_data(
                    uploaded_file
                )
            else:
            ...
            logger.info("   - Inseridos: {insert_stats['inserted']}")
            logger.info("   - Duplicatas ignoradas: {insert_stats['duplicates']}")
            logger.info("   - Erros: {insert_stats['errors']}")
            logger.info("   - Total processado: {insert_stats['total']}")

            # Retorna processo atualizado com estatísticas
            result_serializer = DataImportProcessSerializer(process)

            # Cria mensagem detalhada
            message_parts = ["{insert_stats['inserted']} novos registros adicionados"]
            if insert_stats["duplicates"] > 0:
                message_parts.append(
                    "{insert_stats['duplicates']} duplicatas ignoradas"
                )
            if insert_stats["errors"] > 0:
                message_parts.append("{insert_stats['errors']} erros")
...
class ToggleStatusView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"success": True, "message": message, "process": serializer.data},
                status=status.HTTP_200_OK,
            )
            ...
            return Response(
                {"error": "Erro ao alterar status: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class DataPreviewView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"error": "Erro ao buscar prévia: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            ...
            for process in active_processes:
                ...
                try:
                    ...
                    logger.error(
                        f"Erro ao buscar no processo {process.table_name}: {e}"
                    )
                    continue
            ...
            return Response(
                {"error": "Erro ao buscar dados: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class PublicDownloadDataView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            ...
            if columns_param:
                selected_columns = [
                    col for col in columns_param.split(",") if col in all_columns
                ]
            else:
            ...
            if filters_param:
                try:
                    ...
                    logger.info(
                        "[DOWNLOAD] Filtros parseados com sucesso: {len(active_filters)} filtros"
                    )
                    logger.info("[DOWNLOAD] Detalhes dos filtros: {active_filters}")
                except (json.JSONDecodeError, ValueError) as e:
                    logger.warning(
                        "[DOWNLOAD] Erro ao parsear filtros JSON: {filters_param}"
                    )
                    logger.warning("[DOWNLOAD] Erro: {str(e)}")
            else:
            ...
            def apply_filters_to_record(record_data, filters):
                ...
                for column_name, filter_config in filters.items():
                    ...
                    if filter_type == "string":
                        cell_str = (
                            str(cell_value).lower() if cell_value is not None else ""
                        )
                        ...
                        if filter_operator == "contains" and filter_str not in cell_str:
                            ...
                            and not cell_str.startswith(filter_str)
                        ):
                            return False
                        elif filter_operator == "endsWith" and not cell_str.endswith(
                            filter_str
                        ):
                            return False
                        ...
                        try:
                            cell_num = (
                                float(cell_value) if cell_value is not None else 0
                            )
                            ...
                            if filter_operator == "equals" and cell_num != filter_num:
                                ...
                                filter_operator == "greaterThan"
                                and cell_num <= filter_num
                            ):
                                return False
                            elif (
                                filter_operator == "lessThan" and cell_num >= filter_num
                            ):
                                return False
                            elif (
                                filter_operator == "greaterThanOrEqual"
                                and cell_num < filter_num
                            ):
                                return False
                            elif (
                                filter_operator == "lessThanOrEqual"
                                and cell_num > filter_num
                            ):
                                return False
                        ...
                        selected_values = (
                            filter_value if isinstance(filter_value, list) else []
                        )
                        if selected_values and str(cell_value).lower() not in [
                            v.lower() for v in selected_values
                        ]:
                            return False
            ...
            if file_format == "csv":
                ...
                response = StreamingHttpResponse(
                    iter_csv_rows(), content_type="text/csv; charset=utf-8"
                )
                response["Content-Disposition"] = (
                    f'attachment; filename="{process.table_name}.csv"'
                )
                ...
                logger.info(
                    "[DOWNLOAD] CSV gerado - Total de registros processados: {total_count}, Filtrados: {filtered_count}"
                )
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="{process.table_name}.xlsx"'
                )

                logger.info(
                    "[DOWNLOAD] XLSX gerado - Total de registros processados: {total_count}, Filtrados: {filtered_count}"
                )

            logger.info("[DOWNLOAD] Download concluido com sucesso")
            logger.info("=" * 60)
...
class PublicDataPreviewView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            ...
            if not columns:
                return Response(
                    {"success": False, "error": "Estrutura de colunas não disponível"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"success": False, "error": "Erro ao buscar dados: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class PublicColumnMetadataView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
                ...
                columns_metadata.append(
                    {
                        ...
                        "unique_values": (
                            unique_values if filter_type == "category" else []
                        ),
                    }
                )
            ...
            return Response(
                {"success": False, "error": "Erro ao buscar metadados: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Total records
            total_records = (
                DataImportProcess.objects.aggregate(Sum("record_count"))[
                    "record_count__sum"
                ]
                or 0
            )
            ...
            monthly_data = (
                DataImportProcess.objects.filter(created_at__gte=six_months_ago)
                .annotate(month=TruncMonth("created_at"))
                .values("month")
                .annotate(count=Count("id"), total_records=Sum("record_count"))
                .order_by("month")
            )
            ...
            for item in monthly_data:
                ...
                volume_gb = ((item["total_records"] or 0) * 1024) / (
                    1024 * 1024 * 1024
            ...
            for i in range(6):
                ...
                if month_key in data_by_month:
                    ...
                    monthly_volume.append(
                        {"month": month_name, "value": 0, "datasets": 0}
                    )
            ...
            avg_volume = (
                sum(item["value"] for item in monthly_volume) / len(monthly_volume)
                if monthly_volume
                else 0
            )
            ...
            return Response(
                {
                    ...
                    "data": {
                        "metrics": {
                            "total_datasets": status_counts["active"]
                            ...
                            "storage_percent": min(
                                int((storage_tb / 24) * 100), 100
                            ),  # Assuming 24TB capacity
...
class TaskStatusView(APIView):
    ...
    def get(self, request, task_id):
        ...
        try:
            ...
            return Response(
                {"error": "Erro ao verificar status da task", "error_id": error_id},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
